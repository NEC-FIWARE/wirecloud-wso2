...
class WSO2ViewTestCase(TestCase):
    ...
    def test_oauth_discovery(self):
        from wirecloud.wso2 import views
        reload(views)

        backend_mock = MagicMock()
        backend_mock.AUTHORIZATION_URL = 'https://wso2.com/oauth2/authorize'
        backend_mock.ACCESS_TOKEN_URL = 'https://wso2.com/oauth2/token'
        ...
        views.HttpResponse.assert_called_once_with(json.dumps({
            ...
            'auth_endpoint': 'https://wso2.com/oauth2/authorize',
            'token_endpoint':'https://wso2.com/oauth2/token',
    ...
    def test_login_not_authenticated(self):
        ...
        views.reverse.assert_called_once_with('social:begin', kwargs={'backend': 'wso2'})
    ...
    @patch('django.conf.settings', new=MagicMock(FIWARE_PORTALS=({'url':'http://wso2.com'},)))
    @patch('django.views.decorators.http.require_GET', new=get_decorator)
    def test_logout_external_domain(self):
        ...
        request = MagicMock()
        request.META = {
            'HTTP_ORIGIN': 'http://wso2.com'
        }
        response = views.logout(request)

        self.assertEqual({
            'Access-Control-Allow-Origin': 'http://wso2.com',
